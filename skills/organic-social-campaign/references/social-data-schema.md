...
| `tracks[].audienceReality` | object | optional | who is **actually on this track's account**, as opposed to who it addresses: `{ "composition": "<plain-language, with a proportion>", "source": "<client analytics or client statement>", "verified": "<YYYY-MM-DD>" }`. Absent → establish it before sign-off; a track aimed at an audience that is not on its channel is the failure this field exists to catch (see [strategy-document-template.md](strategy-document-template.md) Chapter 2) |
| `tracks[].relaysTo[]` | string[] | optional | audience ids this track's audience is expected to carry the message **onward** to. Present ⇒ the track is a **relay** track: content is written as ammunition for the intermediary, and its success metric is forwarding (shares/saves/reposts), not conversion |
| `tracks[].brand_ref` | string | optional | **which brand fronts this track's assets** — a brand-folder slug. Absent ⇒ the client's own brand. Set it when a track posts on a campaign/coalition account whose brand is not the client's: every asset on that track renders from `brand_ref`'s folder, visual continuity with that account's existing feed is required, and the two brands are never co-styled in one asset (SKILL.md Phase 1 Step 2 "Per-track brand identity") |
