...
10. **Library integrity across stages.** Gates 1–9 govern a claim in *one* deliverable; a multi-stage engagement reuses an evidence library, and reuse is where verification lapses. Verification state lives on the **library entry** (source URL, date, commissioner, independence, geography, and where in the source the figure was found) — downstream stages inherit state, never trust. Re-verify at every stage boundary, resolve internal proof-point IDs to a real source + link before anything leaves the team, and treat **client-supplied collateral as the lowest source tier** (a lead to trace, not a source to cite). Two failure classes citation resolution cannot see — scope-widening and population conflation — plus the retirement and close-out digest procedure: [evidence-library-integrity.md](references/evidence-library-integrity.md).
...
- **Per-track brand identity.** A track posting on a campaign or coalition account fronts **that account's brand**, not the client's: its assets render from the fronting brand's own brand folder (if none exists, build one from public material — the account's live site/feed — and request official guidelines through the client in parallel), **visual continuity with the account's existing feed is a hard requirement**, and the two brands are never co-styled in one asset (originating engagement, 2026-08-17: all consumer-track content moved to the campaign brand while the B2B track stayed on the client brand). Record the fronting brand per track in the campaign manifest alongside `account` (e.g. `tracks[].brand_ref`), so every render resolves the right brand folder without a per-post judgment call.
...
3. **Per edition (default one week):** select + partition posts by
   ...
   the user to run separately (chart/diagram/pdf/pptx); route any `reel`/`short`
   row whose producer is `video-hd` to the branded-video skill as a designed motion
   graphic (name its rhetorical shape, storyboard the frames before rendering, verify
   the delivered file before the gate); caption the edition
