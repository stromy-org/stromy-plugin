---
name: media-guide
description: "Explain and route branded image and video generation: what the media tools make, why the workspace connector and your own provider key are both required, which media-* skill to choose, and what a generated artifact comes back with. Use whenever a client asks how image or video generation works, what the Media Gen connector does, how to register their generation key, or which media-* skill fits a request."
---
<!--
  GENERATED FILE — DO NOT EDIT.
  Owner:       scripts/sync-mcp-skill-stubs.py (via sync-on-mcp-skill-change.yml)
  Source:      MCPs/media-gen-mcp/skills/media-guide/SKILL.md
...
# Branded media guide (MCP-hosted skill)
...
1. Read the main skill instructions:
   → call the `fs_read` tool on the `media-gen` MCP with `path="skills/media-guide/SKILL.md"`.

2. Discover reference files (and any other skill assets), then read on demand:
   → call `fs_list` with `path="skills/media-guide"` (and `path="skills/media-guide/references"`),
   → call `fs_read` with `path="skills/media-guide/references/<file>"`.
